...
encode_kwargs = {"normalize_embeddings": True} # embedder model
...
file_path = os.path.join(os.path.dirname(__file__), "llama-2-70b-chat.Q5_K_M.gguf")
...
Respond by thinking step by step and then giving the name of the action. For example: "Let's think step by step. Since the user already performed action A and their goal is to do B, the next action to take would be C since the description of C says that it does B. Final answer: C." The goal is to detect maliscious actions and infected PCs and restore them.<</SYS>>
\n"""
...
while True: 
    ...
    docs = vectorstore.similarity_search(question) # trazenje relevantnih dijelova dokumentacije
    ...
    prompt = PromptTemplate.from_template("{historyprompt}") # stvaranje prompta za upit LLMu za iducu akciju
    ...
    prompt = PromptTemplate.from_template("{historyprompt}") # prompt za generiranje sazetka
    partial_prompt = prompt.partial(historyprompt=temp2)
